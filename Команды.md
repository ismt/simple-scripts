...
ssh-keygen -o -a 100 -t ed25519 -C "user@domain.org" -f $HOME/.ssh/test && cat "$HOME/.ssh/test.pub"
...
# исполняемый файл с правами 755
install -v -o root -g root -m 755 ./mybinary /usr/local/bin/mybinary

# конфиг только для чтения
install -v -o root -g www-data -m 640 ./app.conf /etc/app/app.conf

# если папка назначения не существует — создать её заодно
install -v -D -m 644 ./file.conf /etc/newdir/file.conf
...
```bash
# С  прогрессом
mysqldump --host=localhost --user=root --password --compress --max_allowed_packet=1000M --extended-insert --lock-tables --quick --skip-triggers dev_db | pv | pzstd -3 -c > /tmp/mysqldump.zstd

pv /tmp/sync/api.sql.zstd | pzstd -d -c | grep -v '999999\-' | mysql --host=8.8.8.8 --user=user_dev --password --compress --skip-ssl --database=dev_api2


# Без прогресса
mysqldump --host=localhost --user=root --password --compress --max_allowed_packet=1000M --extended-insert --lock-tables --quick --skip-triggers dev_db | pzstd -3 -c > /tmp/mysqldump.zstd

cat /tmp/mysqldump.zstd | pzstd -d -c | grep -v '999999\\-' | mysql --host=localhost --user=root --password --compress --database=t


# В 7z
DB_NAME=dev && mysqldump --host=localhost --user=root --compress --password --max_allowed_packet=1000M --extended-insert --lock-tables --quick --skip-triggers "$DB_NAME" | 7z a -si -mx=1 /tmp/mysqldump_$DB_NAME.7z


# Стандарт 
DB_NAME=dev && mysqldump --host=localhost --user=root --compress --password=ttt --max_allowed_packet=1000M --extended-insert --lock-tables --quick --skip-triggers "$DB_NAME" | gzip > /tmp/mysqldump_$DB_NAME.sql.gz

gzip -d -c /tmp/mysqldump.sql.gz | grep -v '999999\\-' | mysql --host=localhost --user=root --password --compress --database=DB_NAME
...
В `GRUB_CMDLINE_LINUX_DEFAULT` удалить `quiet splash`. Или:
...
### Перезагрузить gunicorn или uwsgi, послать int только дочерним процессам
