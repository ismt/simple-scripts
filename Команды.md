...
```bash
# С  прогрессом
mysqldump --host=localhost --user=root --password --compress --max_allowed_packet=1000M --extended-insert --lock-tables --quick --skip-triggers dev_db | pv | pzstd -3 -c > mysqldump.zstd
...
# Без прогресса
mysqldump --host=localhost --user=root --password --compress --max_allowed_packet=1000M --extended-insert --lock-tables --quick --skip-triggers dev_db | pzstd -3 -c > mysqldump.zstd

cat mysqldump.zstd | pzstd -d -c | grep -v '999999\\-' | mysql --host=localhost --user=root --password --compress --database=t


# В 7z
DB_NAME=dev && mysqldump --host=localhost --user=root --compress --password --max_allowed_packet=1000M --extended-insert --lock-tables --quick --skip-triggers "$DB_NAME" | 7z a -si -mx=1 mysqldump_$DB_NAME.7z


# Стандарт 
DB_NAME=dev && mysqldump --host=localhost --user=root --compress --password=ttt --max_allowed_packet=1000M --extended-insert --lock-tables --quick --skip-triggers "$DB_NAME" | gzip > mysqldump_$DB_NAME.sql.gz

gzip -d -c ./mysqldump.sql.gz | grep -v '999999\\-' | mysql --host=localhost --user=root --password --compress --database=DB_NAME
